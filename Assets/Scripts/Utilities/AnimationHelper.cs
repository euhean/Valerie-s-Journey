...
/// Simple instant text feedback above entities.
/// </summary>
public static class AnimationHelper {
    ...
    public static void ShowStrongAttack(Vector3 worldPos) => ShowText(worldPos, "STRONG!", Color.orange);
    
    // Combo display for Vincent's on-beat attacks
    public static void ShowCombo(Vector3 worldPos, int comboCount) {
        string comboText = $"COMBO {comboCount}/4";
        Color comboColor = Color.Lerp(Color.white, Color.orange, comboCount / 4f);
    ...
    public static void ShowHitFlash(SpriteRenderer renderer, Color flashColor, float duration) {
        ...
        MonoBehaviour context = renderer.GetComponent<MonoBehaviour>();
        if (context != null) {
    ...
    public static void ShowStrongHitShake(Transform target, SpriteRenderer renderer, Color flashColor, float duration) {
        ...
        MonoBehaviour context = target.GetComponent<MonoBehaviour>();
        if (context != null) {
    ...
    private static System.Collections.IEnumerator HitFlashCoroutine(SpriteRenderer renderer, Color flashColor, float duration) {
    ...
    private static System.Collections.IEnumerator StrongHitShakeCoroutine(Transform target, SpriteRenderer renderer, Color flashColor, float duration) {
